...
const pending: RequestHandler = async (_req, res) => {
  try {
    const blogs = await Blog.find({ status: "pending" }).populate("author", "name").sort({ createdAt: -1 }).lean();
    res.json({ blogs });
  } catch (e) {
...
const setStatus = (status: "approved" | "rejected" | "hidden"): RequestHandler => async (req, res) => {
  try {
